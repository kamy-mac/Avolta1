...
import { addSubscriber, sendNewsletterEmail } from './mailchimp';
...
export const subscribeToNewsletter = async (email: string): Promise<void> => {
  try {
    ...
    // If Mailchimp subscription succeeds, store locally
...
export const createPublication = async (data: {
  ...
  if (newPost.status === 'published' && data.sendNewsletter) {
    try {
